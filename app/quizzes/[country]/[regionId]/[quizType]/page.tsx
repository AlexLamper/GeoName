...
import { fetchCountries, fetchPlacesByRegion } from '@/utils/overpass-api';
...
const QuizTypePage = () => {
  const { country, regionId, quizType } = useParams(); // Use regionId from params
  ...
  useEffect(() => {
    const fetchCountryAndPlaces = async () => {
      try {
        ...
        // Fetch places for the given regionId
        const numericRegionId = Number(regionId);
        if (!isNaN(numericRegionId) && numericRegionId > 0 && typeof quizType === 'string') {
          console.log(`Fetching places for region ID: ${numericRegionId} with quiz type: ${quizType}`);
          const fetchedPlaces = await fetchPlacesByRegion(numericRegionId, quizType);
          ...
          if (fetchedPlaces) {
            ...
            console.log(`No places fetched for region ID ${numericRegionId} and quiz type ${quizType}`);
          }
        } else {
          console.error(`Invalid region ID or quiz type provided: ${regionId} / ${quizType}`);
        }
      } catch (error) {
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-6 lg:max-w-[80%] max-w-full">
        ...
        <h1 className="text-4xl font-bold mb-4">
          <span style={{ color: '#1A5319' }}>{countryName || country}</span> - {quizType} in region
        </h1>
